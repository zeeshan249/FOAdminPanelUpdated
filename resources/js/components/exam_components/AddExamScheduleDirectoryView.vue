    ...
    <transition name="fade" mode="out-in">
      ...
      >
        ...
        <template v-slot:item.actions="{ item }">
          ...
          >
          <v-icon
            v-if="item.lms_exam_schedule_is_active_status == 'Active'"
            v-permission="'Edit Exam Schedule'"
            small
            color="error"
          ...
          >
          <v-icon
            v-if="item.lms_exam_schedule_is_active_status == 'Inactive'"
            v-permission="'Edit Exam Schedule'"
            small
            color="success"
            @click="enableDisableExamSchedule(item)"
      ...
      <template>
        <v-card class="mt-8 mx-auto" max-width="600">
          ...
          <v-card-text class="pt-0">
            ...
            <div class="mt-2">
              <div class="float-left">
                ...
                ><strong
                  >Start: &nbsp;{{ lms_exam_schedule_live_from_date }}
                </strong>
              </div>
              ...
              <div class="float-right">
                <v-icon class="ml-2 text-center" small>mdi-calendar</v-icon
                ><strong
                  >End: &nbsp;{{ lms_exam_schedule_live_to_date }}
                </strong>
              </div>
            </div>
            <br />
            ...
            <div class="mt-2">
              <div class="float-left">
                <v-icon class="ml-2" small>mdi-comment-question-outline</v-icon
                ><strong
          ...
          <v-card-text>
            ...
            <span class="caption text-center"
              ><strong
                >Subscription: &nbsp;{{ lms_exam_schedule_is_free_paid_status }}
              </strong>
            </span>
            ...
            <template>
              <v-chip
                ...
                dark
                ><strong>{{ lms_exam_schedule_is_active_live }}</strong></v-chip
              >
            </template>
          </v-card-text>
        </v-card>
...
export default {
  ...
  methods: {
    ...
    viewExamSchedule(item) {
      ...
      this.lms_exam_schedule_live_to_date = item.lms_exam_schedule_live_to_date;
          ...
          .post(
            "web_enable_disable_exam_schedule",
            {
              ...
              isExamScheduleActive:
                item.lms_exam_schedule_is_active_status == "Active" ? 0 : 1,
            },
            this.authorizationConfig
          )
          .then(({ data }) => {
            ...
            if (
              ...
              if (data.responseData == 1) {
                ...
                this.changeSnackBarMessage(
                  this.$t("label_exam_schedule_status_changed")
                );
    ...
    getAllExamSchedule(e) {
      ...
      this.$http
        .get(`web_get_all_exam_schedule?page=${e.page}`, {
          params: postData,
          headers: { Authorization: "Bearer " + ls.get("token") },
        })
        .then(({ data }) => {
