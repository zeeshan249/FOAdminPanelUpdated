    ...
    <transition name="fade" mode="out-in">
      ...
      >
        ...
        <template v-slot:item.lms_student_registration_type="{ item }">
          <v-chip
            x-small
            :color="getStatusColor(item.lms_student_registration_type)"
            dark
        ...
        <template v-slot:item.actions="{ item }">
          <v-icon
            v-permission="'Delete Student' | 'Add Student' | 'Edit Student'"
            small
            class="mr-2"
          ...
          <v-icon
            v-if="item.lms_student_is_active == 'Active'"
            v-permission="'Delete Student'"
            small
            color="error"
...
export default {
  ...
  data() {
    return {
      ...
      excelData: [],
      excelFileName:
        "StudentListAsExcel" + "_" + moment().format("DD/MM/YYYY") + ".xls",
    };
  },
  computed: {
  ...
  created() {
    console.log("props", this.userPermissionDataProps);
    // Token Config
