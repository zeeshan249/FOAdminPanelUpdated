  ...
  <div style=" margin:auto; padding:auto; width:1200px;" id="app">
          ...
          <v-list-item two-line>
            <v-list-item-content>
              ...
              <v-list-item-subtitle
                ...
                <v-icon>mdi-forward</v-icon> Post</v-list-item-subtitle
              >
            </v-list-item-content>
          </v-list-item>
        </v-toolbar-title>
    ...
    <v-dialog v-model="DialogPostContent" width="500">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" text @click="DialogPostContent = false">
            Close
          </v-btn>
        </v-card-actions>
      </v-card>
...
export default {
  ...
  data() {
    return {
      ...
      excelData: [],
      excelFileName: "PostList" + "_" + moment().format("DD/MM/YYYY") + ".xls",
    };
  },
  computed: {
  ...
  methods: {
        ...
        this.$http
          .post(
            "web_is_approve_post",
            {
              ...
              isApproved: item.lms_is_post_approved == "1" ? 0 : 1,
            },
            this.authorizationConfig
          )
          .then(({ data }) => {
    ...
    savePDF() {
      ...
      pdfDoc.autoTable({
        columns: [
          { header: "Registration", dataKey: "lms_registration_code" },
          { header: "Name", dataKey: "lms_student_full_name" },
          { header: "Date", dataKey: "lms_student_course_mapping_created_at" },
          { header: "Mobile Number", dataKey: "lms_student_mobile_number" },
      ...
      pdfDoc.save(
        "StudentListAsPDF" + "_" + moment().format("DD/MM/YYYY") + ".pdf"
      );
    },
  },
};
